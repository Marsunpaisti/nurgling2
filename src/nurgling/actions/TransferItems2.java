...
public class TransferItems2 implements Action
{
    ...
    {
        NUtils.debugMsg(gui, "[TransferItems2] start items=" + items);
        ...
        for(String item : resitems) {
            ...
            if(areas != null) {
                NUtils.debugMsg(gui, "[TransferItems2] candidate outputs item=" + item + " thresholds=" + areas);
                for (Double quality : areas.descendingKeySet()) {
                    int matchingItems = getItemsExactMatch(item, quality).size();
                    NUtils.debugMsg(gui, "[TransferItems2] inventory matches item=" + item + " q>=" + quality + " count=" + matchingItems + " area=" + areas.get(quality));
                    if (matchingItems > 0) {
                ...
                NUtils.debugMsg(gui, "[TransferItems2] no output areas for item=" + item);
            }
        }
        NUtils.debugMsg(gui, "[TransferItems2] threshold groups=" + thresholdGroups.keySet());
        ...
        for (ThresholdGroup group : thresholdGroups.values()) {
            ...
            if (group.threshold > 1) {
                ...
                for (String areaId : group.itemsByArea.keySet()) {
                    NUtils.debugMsg(gui, "[TransferItems2] process threshold=" + group.threshold + " area=" + areaId + " transfers=" + group.itemsByArea.get(areaId).size());
                ...
                while (!remaining.isEmpty()) {
                    ...
                    NUtils.debugMsg(gui, "[TransferItems2] process nearest area=" + nearestAreaId + " transfers=" + remaining.get(nearestAreaId).size());
        ...
        NUtils.debugMsg(gui, "[TransferItems2] complete");
    ...
    private void processAreaTransfers(String areaId, List<ItemTransfer> itemsForArea, NGameUI gui) throws InterruptedException {
        for (ItemTransfer itemTransfer : itemsForArea) {
            NUtils.debugMsg(gui, "[TransferItems2] resolve storages area=" + areaId + " item=" + itemTransfer.itemName + " q=" + itemTransfer.quality);
            ArrayList<NContext.ObjectStorage> storages = cnt.getOutStorages(itemTransfer.itemName, itemTransfer.quality);
            NUtils.debugMsg(gui, "[TransferItems2] storages area=" + areaId + " item=" + itemTransfer.itemName + " count=" + storages.size());
            for (NContext.ObjectStorage output : storages) {
                if (output instanceof NContext.Pile) {
                    NUtils.debugMsg(gui, "[TransferItems2] transfer to pile area=" + areaId + " item=" + itemTransfer.itemName +
                ...
                if (output instanceof Container) {
                    ...
                    NUtils.debugMsg(gui, "[TransferItems2] transfer to container area=" + areaId + " item=" + itemTransfer.itemName +
                ...
                if (output instanceof NContext.Barter) {
                    NUtils.debugMsg(gui, "[TransferItems2] transfer to barter area=" + areaId + " item=" + itemTransfer.itemName +
    ...
    private String findNearestArea(Set<String> areaIds, NGameUI gui) {
        ...
        for (String areaId : areaIds) {
            ...
            NUtils.debugMsg(gui, "[TransferItems2] distance candidate area=" + areaId + " dist=" + dist);
            if (dist < minDist) {
                minDist = dist;
                nearest = areaId;
            }
        ...
        if (nearest == null && !areaIds.isEmpty()) {
            ...
            NUtils.debugMsg(gui, "[TransferItems2] no reachable area by distance; fallback area=" + nearest);
        }

        NUtils.debugMsg(gui, "[TransferItems2] nearest area=" + nearest + " dist=" + minDist);
